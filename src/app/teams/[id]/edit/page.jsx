...
import { ArrowLeft, Save, Users, UserPlus, Building2, Eye } from 'lucide-react';
...
export default function EditTeam() {
  ...
  const fetchTeam = async () => {
    try {
      const data = await backendApi.get(`/teams/${teamId}`);
      setFormData({
        name: data.name || '',
  ...
  const fetchTeams = async () => {
    try {
      ...
      // Filter out current team to prevent self-reference
      const otherTeams = data.filter(team => team.id !== parseInt(teamId));
      setTeams(otherTeams || []);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await backendApi.put(`/teams/${teamId}`, formData);
  ...
  const viewHierarchy = () => {
    router.push(`/teams/${teamId}/hierarchy`);
  };
